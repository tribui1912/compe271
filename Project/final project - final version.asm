...
while_level:
bgt $t0, MAX_LEVEL, out_while_level   # if (level > MAX_LEVEL), exit the loop
li	$t9, 0		# position = 0
...
check_max_level:
 	bgt $t0, MAX_LEVEL, out_while_level
 	j while_1
        
while_1:  # while(1)
        li  $v0, 12  # Read a character
        syscall
        beq $v0, 'a', move_left  # if (input == 'a')
        beq $v0, 'A', move_left  # if (input == 'A')

        beq $v0, 'd', move_right  # if (input == 'd')
        beq $v0, 'D', move_right  # if (input == 'D')

        beq $v0, 10, next_level      # else if (input == 10)
        j    while_1

move_left:
        addi $t9, $t9, -1  # position - 1
        bge  $t9, $zero, call_dis_func
        addi $t9, $t9, NUM_EQUATIONS  # position = (position - 1 + NUM_EQUATIONS)
        j    call_dis_func

move_right:
        addi $t9, $t9, 1  # position + 1
        blt  $t9, NUM_EQUATIONS, call_dis_func
        sub  $t9, $t9, NUM_EQUATIONS  # position = (position + 1) % NUM_EQUATIONS
        j    call_dis_func
...
add	$s6, $s6, $s5
lw	$a0, 0($s6)	# load address of equations[i].equation;
li	$v0, 4		# print string
syscall
